...
;; start of :web component cause start of :db an :cache components, respectively
(ctx/start! *ctx :web)
;; #unifier.response.UnifiedSuccess{:type :unifier.response/success, :data :web, :meta nil}
...
*Warning!*  Cyclic dependency check between components is not implemented yet. 
If stack overflow error occurs during the start of component then there is cyclic dependency. :)
